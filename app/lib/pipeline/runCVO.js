...
export async function runCVO(input) {
  if (input?.vision != null) {
    ...
    return {
      ...
      semanticDraft: null,
      sceneInventory: input?.sceneInventory ?? null,
      retrievalBriefQueue: input?.retrievalBriefQueue ?? null,
  ...
  if (!response.ok) {
    return {
      ...
      sceneInventory: payload?.sceneInventory ?? null,
      retrievalBriefQueue: payload?.retrievalBriefQueue ?? null,
      cvo_confidence: 0,
